	...
	select concept_id, drug_name_original, concept_name, string_agg(word, ' ' order by word) as ingredient_list  from (
		...
		from (
			...
			from (
				select distinct drugname as drug_name_original, cast(null as varchar) as concept_name, cast(null as integer) as concept_id, null as update_method
				from medeffect_drugs
	            where drugname not like '%/%' and drugname not like '% AND %' and drugname not like '% WITH %' and drugname not like '%+%'
				) aa
			order by concept_name desc
		) bb
	) cc
...
select distinct drug_name_original, nda_ingredient, concept_id, update_method
from (
	select distinct drugname as drug_name_original, null as nda_ingredient, cast(null as integer) as concept_id, null as update_method
	from medeffect_drugs
) aa;
...
select distinct drug_product_id, drug_name_original, lookup_value, concept_id, update_method
from (
	select distinct drug_product_id, drugname as drug_name_original, cast(null as varchar) as lookup_value, cast(null as integer) as concept_id, cast(null as varchar) as update_method
	from medeffect_drugs
) aa ;
