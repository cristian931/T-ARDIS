...
'''This script is used to clean the different drug-target databases, the idea is to have as final output a pairwise relationship between the drug and the targets as Uniprot ids'''
...
# DRUGBANK: From Drugbank two different file have been downloaded, the drugbank vocabulary, containing the drugs names and their respective codes and the Target file, containg the information of the drugs' targets and the relationship with the former 
...
drugbank_merged.to_csv('Drugbank_relationship.input', sep='\t', index=False)


# DGiDB: unique File from which extract the information. However the conversion from Gene name to UNIPROT id is needed. the latter has been downloaded from uniprot searching for the human proteome and selecting the uniprot entries together with the gene names and also the gene names synonyms
...
dgidb_interactions['drug_claim_primary_name'] = dgidb_interactions['drug_claim_primary_name'].apply(lambda x: x.split(', '))
...
uniprot = pd.read_csv('human_proteome_gene_names_11_03.tab', sep='\t', names=['Uniprot ID', 'Primary Gene', 'Synonym'], header=0).fillna('')
...
dgidb_merged = pd.merge(dgidb_interactions, uniprot_exploded, left_on=['gene_claim_name'], right_on=['Gene names'], how='left').dropna().drop(columns=['gene_claim_name'])
dgidb_merged.to_csv('DGidb_relationships.input', sep='\t', index=False)
...
n1= drugcentral.explode('ACCESSION')

n1.to_csv('drugcentral_merged.input', sep='\t', index=False)
